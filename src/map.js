...
// ─── GeoJSON builders ────────────────────────────────────────────────────────
function buildHeatmapGeoJson(tracks) {
...
function buildTrackGeoJson(tracks) {
    return {
...
function applyTracks(tracks) {
    ...
    const heatmapSource = map.getSource(HEATMAP_SOURCE);
    const trackSource = map.getSource(TRACK_SOURCE);
    if (heatmapSource && trackSource) {
        ...
        setupLayers();
        map.getSource(HEATMAP_SOURCE).setData(buildHeatmapGeoJson(tracks));
        map.getSource(TRACK_SOURCE).setData(buildTrackGeoJson(tracks));
    }
}
...
export function setVisibleTypes(types) {
    const visible = types === null
        ? storedTracks
